...
\subsection{Simulaciones y ruido} 
 Una vez que ya hemos visto las puertas que podemos usar en un circuito cuántico, solo nos queda entender como se ejecutan estos programas cuánticos, aunque de alguna manera aún no sabemos ni que forma tienen, ni como se representan estos circuitos. Si que podemos entender el concepto de las distintas opciones. (qiskit, intro cuantica)\newline

 Para ejecutar estos programas vamos a tener dos posibilidad:
 \begin{itemize}
     \item \textbf{Simulación}, en nuestro caso con Qiskit. Esta posibilidad nos va a ofrecer una simulación teórica de lo que ocurre en nuestro circuito. Será ejecutado en un ordenador clásico y nos aa a permitir obtener resultados con seguridad y sin ningún problema asociado de errores o ruido.
     \item \textbf{Ordenador cuántico}, utilizaremos los ordenadores de IBM. La tecnología para la creación de ordenadores cuánticos más fieles sigue avanzando. Esta intenta mitigar los errores que tiene cada operación, así como los errores relacionados con el ruido del entorno influyen en el estado del sistema. Cada ordenador tiene si propia estructura y sus estudios sobre 
 \end{itemize}

 Ambas opciones utilizan el mismo mecanismo, repiten el proceso tantas veces como les sea requerido y acumulan los resultados, mostrando una acumulación 
 
 Por ello vamos a realizar un ejemplo muy sencillo que nos permita introducir el tipo de ejecución y además entender que es el ruido en una máquina cuántica real. \newline
