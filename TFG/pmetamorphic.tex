...
\textbf{Implementación}: Vamos a comparar los resultados entre la simulación $P(f)$ con $P(f\circ g)$. En este caso hemos decidido utilizar un automorfismo particular a la hora de realizar las pruebas. Sea $g: \{0,1\}^{n} \rightarrow \{0,1\}^{n}$ talque aplica la puerta $X$ a cada componente, es decir, $g(\mathbf{x})=\mathbf{1}-\mathbf{x}$ que es el complementario binario de $\mathbf{x}$.\newline
...
\textbf{Regla II}: Sean $\mathbf{s}$ y $\mathbf{s}'$ cadenas binarias de longitud $n$. Entonces $P(\mathbf{s})\oplus P(\mathbf{s}')=\mathbf{s} \oplus \mathbf{s}'$.\newline
 ...
 El circuito utilizado es idéntico al que se muestra en la {Figura } que se empleó en el caso particular de esta regla. \newline
